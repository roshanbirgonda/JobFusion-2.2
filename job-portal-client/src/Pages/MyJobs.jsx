...
const MyJobs = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/myJobs/test1@gmail.com`);
      ...
      {currentJobs.map((job,index) => (
              ...
              <tr key={index}>
                ...
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-s whitespace-nowrap p-4">
                  ...
                  <button className='hover:underline duration-500'><Link to={`/edit-job/${job?._id}`}>Edit</Link></button>
                </td>
